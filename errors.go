...
import (
	...
	"github.com/ikaiguang/protoc-gen-go-errors/errors"
)
...
func genErrorsReason(_ *protogen.Plugin, _ *protogen.File, g *protogen.GeneratedFile, enum *protogen.Enum) bool {
	// http code start
	httpCodeSuffix := "_http_code"
	g.P("var ", enum.Desc.Name(), httpCodeSuffix, " = map[string]int{")
	...
	for _, v := range enum.Values {
		...
		originComment := string(v.Comments.Leading)
		comment := v.Comments.Leading.String()
		if comment == "" {
			comment = v.Comments.Trailing.String()
			originComment = string(v.Comments.Trailing)
		}
		originComment = fmt.Sprintf("%q", strings.TrimSpace(originComment))
		...
		// http code
		g.P("\"", v.Desc.Name(), "\" : ", enumCode, ",")
	}

	// http code end
	g.P("}")
	// http code
	enumComment := enum.Comments.Leading.String()
	if enumComment == "" {
		enumComment = enum.Comments.Trailing.String()
	}
	g.P(strings.TrimSpace(enumComment))
	g.P("func (x ", enum.Desc.Name(), ") HTTPCode() int {")
